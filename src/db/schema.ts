...
ON goal_contributions(goal_id, date);`,
 ...
 ON weekly_boosts(mission_id, code);`,
  ...
  // Migration 2 : Ajout de la table closures (clôtures mensuelles)
  // Note : les anciennes migrations 2-11 sont déjà intégrées dans la migration 1.
  2: [
    `CREATE TABLE IF NOT EXISTS closures (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      month INTEGER NOT NULL,                  -- 1..12
      year INTEGER NOT NULL,
      theoretical_balance INTEGER NOT NULL,    -- solde calculé par l’app
      physical_balance INTEGER NOT NULL,       -- solde réel saisi par l’utilisateur
      difference INTEGER NOT NULL,             -- physical - theoretical
      transaction_id INTEGER,                  -- id de la transaction d’ajustement créée
      note TEXT,
      created_at TEXT NOT NULL,
